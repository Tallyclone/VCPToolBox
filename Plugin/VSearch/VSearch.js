...
const {
  VSearchKey: VSEARCH_KEY,
  VSearchUrl: VSEARCH_URL,
  GroundingSearchKey: GROUNDING_SEARCH_KEY,
  GroundingSearchUrl: GROUNDING_SEARCH_URL,
  GrokSearchKey: GROK_SEARCH_KEY,
  GrokSearchUrl: GROK_SEARCH_URL,
  VSearchModel: MODEL,
...
const KIMI_MAX_RESULTS = Math.min(
  Math.max(parseInt(KIMI_SEARCH_MAX_RESULTS, 10) || 5, 1),
  20
);
const KIMI_INCLUDE_CONTENT = KIMI_SEARCH_INCLUDE_CONTENT === "true";

const resolveModeApiConfig = (mode) => {
  if (mode === "grounding") {
    return {
      apiKey: GROUNDING_SEARCH_KEY || VSEARCH_KEY,
      apiUrl: GROUNDING_SEARCH_URL || VSEARCH_URL,
    };
  }

  if (mode === "grok") {
    return {
      apiKey: GROK_SEARCH_KEY || VSEARCH_KEY,
      apiUrl: GROK_SEARCH_URL || VSEARCH_URL,
    };
  }
...
const resolveRedirect = async (url) => {
  ...
  try {
    ...
    if (
      ...
      !fallbackUrl.includes("grounding-api-redirect")
    ) {
      return fallbackUrl;
    }
...
const callGroundingMode = async (topic, keyword, showURL = false) => {
  const { apiKey, apiUrl } = resolveModeApiConfig("grounding");
...
${
  showURL
    ? "5. 严格溯源：每一条重要信息必须附带来源 URL。如果你使用了引用标记（如 [cite: X]），请确保在回复末尾的 [参考来源] 部分列出这些标记对应的完整 URL。"
    : "5. 节省Token：除非特别重要，否则不需要在正文中列出 URL 链接。"
}`;
  ...
  if (!apiKey || !apiUrl) {
    return `[搜索失败] 关键词: ${keyword}。错误原因: Grounding 模式需要配置 GroundingSearchKey/GroundingSearchUrl 或回退的 VSearchKey/VSearchUrl。`;
  }
  ...
  try {
    ...
    if (!content.trim() && toolCalls.length > 0) {
      const toolNames = toolCalls
        ...
        .join(", ");
      log(
        `[Grounding] 上游接口仅返回 tool_calls，未返回最终内容。finish_reason=${
          choice.finish_reason || "unknown"
        }, tools=${toolNames || "unknown"}`
      );
      return `[搜索失败] 关键词: ${keyword}。错误原因: 当前 Grounding 接口只返回了工具调用（${
        toolNames || "unknown"
      }），没有继续执行并产出最终答案。请检查你配置的 GroundingSearchUrl / GroundingSearchKey 对应服务是否真正支持服务端执行 googleSearch 工具。`;
    }

    try {
      const metadata =
        message?.grounding_metadata ||
        response.data.choices[0]?.grounding_metadata;

      const vertexUrlRegex =
        /(?:https?:\/\/)?vertexaisearch\.cloud\.google\.com\/grounding-api-redirect\/[\w\-=]+/g;
      const foundUrls = content.match(vertexUrlRegex) || [];

      const metadataUrls =
        metadata && metadata.grounding_chunks
...
const callGrokMode = async (topic, keywordList) => {
  const { apiKey, apiUrl } = resolveModeApiConfig("grok");
...
请针对上述关键词执行联网搜索，并结合研究主题给出深度总结。`;
  ...
  const cleanGrokText = (text) =>
    (text || "").replace(/<think>[\s\S]*?<\/think>/g, "").trim();

  if (!apiKey || !apiUrl) {
    return "[Grok 配置错误] 请配置 GrokSearchKey/GrokSearchUrl，或提供回退的 VSearchKey/VSearchUrl。";
  }

  const fetchNonStreamFallback = async () => {
    log("[Grok] 流式结果为空，尝试非流式兜底请求...");

    const fallbackResponse = await axios.post(apiUrl, buildPayload(false), {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Content-Type": "application/json",
      },
    ...
    const messageContent =
      typeof choice.message?.content === "string"
        ...
        : extractTextFromParts(choice.message?.content);
    const reasoningContent =
      typeof choice.message?.reasoning_content === "string"
        ? choice.message.reasoning_content
        : "";

    return cleanGrokText(messageContent || reasoningContent || "");
  };

  try {
    log(`[Grok] 正在执行全量搜索 (关键词数量: ${keywordList.length})...`);
    const response = await axios.post(apiUrl, buildPayload(true), {
      headers: {
        Authorization: `Bearer ${apiKey}`,
        "Content-Type": "application/json",
      },
    ...
    return await new Promise((resolve, reject) => {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (typeof delta.content === "string") {
              ...
              fullContent += extractTextFromParts(delta.content);
            }

            if (typeof choice.message?.content === "string") {
              finalMessageContent = choice.message.content;
            } else if (Array.isArray(choice.message?.content)) {
              finalMessageContent = extractTextFromParts(
                choice.message.content
              );
            }

            if (typeof choice.message?.reasoning_content === "string") {
              finalReasoningContent = choice.message.reasoning_content;
      ...
      response.data.on("end", async () => {
        try {
          const rawContent =
            fullContent || finalMessageContent || finalReasoningContent || "";
          const cleanedContent = cleanGrokText(rawContent);
          ...
          const fallbackContent = await fetchNonStreamFallback();
          resolve(fallbackContent || "[Grok 返回空内容]");
        } catch (fallbackError) {
          log(`[Grok] 非流式兜底失败: ${fallbackError.message}`);
          resolve("[Grok 返回空内容]");
        }
      });
    ...
    log(`[Grok] 搜索失败: ${error.message}`);
    return `[Grok 搜索失败] ${error.message}`;
  }
};
...
const callTavilySearch = async (query, tavilyKeyStr) => {
  ...
  if (!apiKey) {
    throw new Error(
      "TavilyKey 未配置，请在根目录 config.env 中设置 TavilyKey。"
    );
  }
...
const callTavilyMode = async (topic, keywordList, tavilyKeyStr) => {
  ...
  try {
    log(
      `[Tavily] 阶段1/2: 正在并发获取 ${keywordList.length} 个关键词的搜索结果 (直接调用 Tavily API)...`
    );
  ...
  // === 阶段2: 模型总结 ===
  const summaryKey = SUMMARY_KEY || VSEARCH_KEY;
  const summaryUrl = SUMMARY_URL || VSEARCH_URL;
  const summaryModel = SUMMARY_MODEL || TAVILY_MODEL || "claude-sonnet-4-6";

  if (!summaryKey || !summaryUrl) {
    log(
      `[Tavily] 未配置总结用 LLM API（SummaryKey/SummaryUrl 或 VSearchKey/VSearchUrl），跳过总结阶段，直接返回原始结果`
    );
  ...
  try {
    log(
      `[Tavily] 阶段2/2: 正在使用 ${summaryModel} 通过 ${summaryUrl} 进行全量总结...`
    );
    ...
    const errorDetail = summaryError.response?.data
      ? JSON.stringify(summaryError.response.data).substring(0, 500)
...
const callKimiSearch = async (
  ...
  const url = baseUrl.endsWith("/search")
    ? baseUrl
    ...
    const response = await axios.post(
      ...
      },
      {
        headers: {
          Authorization: `Bearer ${apiKey}`,
          "Content-Type": "application/json",
          "X-Msh-Tool-Call-Id": `vsearch-${Date.now()}-${Math.random()
            .toString(36)
            .substr(2, 9)}`,
        },
        timeout: 180000,
 ...
 */
const callKimiSearchMode = async (
  topic,
  keywordList,
  apiKey,
  baseUrl,
  ...
  try {
    log(
      `[KimiSearch] 阶段1/2: 正在并发获取 ${keywordList.length} 个关键词的搜索结果...`
    );
    const searchPromises = keywordList.map(async (kw) => {
      const result = await callKimiSearch(
        kw,
        apiKey,
        baseUrl,
        maxResults,
    ...
    combinedResults = allSearchResults.join("\n---\n");
    log(
      `[KimiSearch] 阶段1/2 完成: 搜索结果总长度 ${combinedResults.length} 字符`
    );
  } catch (searchError) {
  ...
  if (!summaryKey || !summaryUrl) {
    log(
      `[KimiSearch] 未配置总结用 LLM API（SummaryKey/SummaryUrl 或 VSearchKey/VSearchUrl），跳过总结阶段，直接返回原始结果`
    );
    return combinedResults;
  }
  ...
  try {
    log(
      `[KimiSearch] 阶段2/2: 正在使用 ${summaryModel} 通过 ${summaryUrl} 进行全量总结...`
    );
    const summaryPayload = {
      model: summaryModel,
      messages: [
...
async function main(request) {
  ...
  if (!SearchTopic || !Keywords) {
    return sendResponse({
      status: "error",
      error: "缺少必需参数: SearchTopic 或 Keywords。",
    });
  }
  ...
  log(
    `启动 VSearch [模式: ${SearchMode}]。主题: "${SearchTopic}"，关键词数量: ${keywordList.length}`
  );
  ...
  if (SearchMode === "tavily") {
    ...
    if (!tavilyKeyStr) {
      return sendResponse({
        status: "error",
        error: "Tavily 模式需要在根目录 config.env 中配置 TavilyKey。",
      });
    }
    const result = await callTavilyMode(SearchTopic, keywordList, tavilyKeyStr);
    return sendResponse({
      status: "success",
      result: `## VSearch 检索报告 [模式: Tavily]\n\n**研究主题**: ${SearchTopic}\n\n${result}`,
    });
  }
  ...
  if (SearchMode === "kimisearch") {
    ...
    if (!KIMI_SEARCH_KEY) {
      return sendResponse({
        status: "error",
        error: "KimiSearch 模式需要在 config.env 中配置 KimiSearchKey。",
      });
    }
    if (!KIMI_SEARCH_URL) {
      return sendResponse({
        status: "error",
        error: "KimiSearch 模式需要在 config.env 中配置 KimiSearchUrl。",
      });
    }
    const result = await callKimiSearchMode(
      SearchTopic,
      keywordList,
      KIMI_SEARCH_KEY,
      KIMI_SEARCH_URL,
      KIMI_MAX_RESULTS,
      KIMI_INCLUDE_CONTENT
    );
    return sendResponse({
      status: "success",
      result: `## VSearch 检索报告 [模式: KimiSearch]\n\n**研究主题**: ${SearchTopic}\n\n${result}`,
    });
  }
  ...
  for (let i = 0; i < keywordList.length; i += CONCURRENCY) {
    ...
    const promises = chunk.map((kw) =>
      callGroundingMode(SearchTopic, kw, showURL)
    );
