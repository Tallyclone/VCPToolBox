...
class ShadowDistributedRouter {
  ...
  syncDeviceAliasesFromRegistries() {
    ...
    for (const serverId of knownServerIds) {
      ...
      if (!normalizeAliasForExactMatch(systemFields.serverName)) {
        continue;
      }

      const existing = findMatchingDeviceByServerName(
        this.deviceAliases.devices,
        systemFields.serverName
      );
      if (existing === false) {
        continue;
      }
      ...
      if (!existing) {
        ...
        changed = true;
        console.log(
          `[ShadowDistributedRouter] device alias auto-created by serverName=${
            systemFields.serverName
          }; serverId=${serverId}; reportedIPs=${systemFields.lastKnownIPs.join(
            ","
          )}`
        );
        continue;
      ...
      if (deviceChanged || removedDuplicates > 0 || purgedRegistryAliases > 0) {
        ...
        changed = true;
        console.log(
          `[ShadowDistributedRouter] device alias synced by serverName=${
            systemFields.serverName
          }: ${previousServerId} -> ${
            systemFields.serverId
...
function mergeUserOwnedDeviceFields(current, latestOnDisk) {
  ...
  for (const device of current.devices || []) {
    const latest = findMatchingDeviceByServerName(
      latestDevices,
      device.serverName
    );
...
function findMatchingDeviceByServerName(devices, serverName) {
  if (!Array.isArray(devices)) return null;

  const normalizedServerName = normalizeAliasForExactMatch(serverName);
  if (!normalizedServerName) return null;

  const candidates = devices.filter(
    (device) =>
      normalizeAliasForExactMatch(device?.serverName) === normalizedServerName
  );
  if (candidates.length === 1) return candidates[0];

  if (candidates.length > 1) {
    console.warn(
      `[ShadowDistributedRouter] serverName device match ambiguous for serverName=${serverName}; matched=${candidates
        .map((device) => device.serverId || device.hostName || "<unknown>")
        .join(",")}. Skipping alias sync to avoid merging devices.`
    );
    return false;
  }
...
function getOnlineDeviceServerId(device) {
  ...
  const match = findMatchingDeviceByServerName(
