...
function initialize(config) {
  ...
  runtimeConfig = {
    ...
    dynamicTemplate:
      (config.BRIDGE_DYNAMIC_TEMPLATE || "off").toLowerCase() === "on",
  };

  startServer();
  console.log(
    `[VCPBridgeServer] Initialized. Hijack mode: ${runtimeConfig.hijackMode}, Port: ${runtimeConfig.port}, Upstream: ${runtimeConfig.upstreamUrl}`
  );
}
  ...
  if (downstreamFormat === "anthropic" && endpoint.type === "chat") {
    ...
    if (stream && upstreamResponse.body) {
      ...
      if (!contentType.includes("text/event-stream")) {
        return sendAnthropicStreamFromOpenAIJson(
          res,
          upstreamResponse,
          fallbackModel
        );
      }
      return sendAnthropicStreamFromOpenAI(
        res,
        upstreamResponse,
        fallbackModel
      );
