...
const DESCRIPTION = Object.freeze({
    ...
    dynamicTemplate: '动态提示词解析引擎模板开关。开启后用原始 prompt 里的编辑器环境状态替换自定义 Prompt 中的占位符（如 {{workspacePath}}），依赖插件目录下的 prompt-mappings.json。',
...
function normalizeBridgeConfig(raw = {}, fallbackEnv = {}) {
    ...
    return {
        ...
        dynamicTemplate: normalizeBoolean(raw.dynamicTemplate ?? raw.BRIDGE_DYNAMIC_TEMPLATE ?? fallbackEnv.BRIDGE_DYNAMIC_TEMPLATE, DEFAULT_BRIDGE_CONFIG.dynamicTemplate),
...
function buildConfigFromEnv(env = {}) {
    return normalizeBridgeConfig({
        ...
        BRIDGE_DYNAMIC_TEMPLATE: env.BRIDGE_DYNAMIC_TEMPLATE,
...
function toEnvCompatConfig(config) {
    ...
    return {
        ...
        BRIDGE_DYNAMIC_TEMPLATE: normalized.dynamicTemplate ? 'on' : 'off',
