...
// --- Configuration (from environment variables set by Plugin.js and config.env) ---
...
const DMX_API_CONFIG = {
  BASE_URL: process.env.BASE_URL || "https://www.dmxapi.cn", // API Host, configurable in config.env
  IMAGE_GENERATION_ENDPOINT: "/v1/images/generations", // New API Endpoint
  MODEL_ID: process.env.MODEL_ID || "doubao-seedream-4-5-251128", // Model ID, configurable in config.env
  DEFAULT_PARAMS: {
...
function collectImageInputs(args) {
  ...
  const indexedKeys = Object.keys(args)
    .map((key) => {
      const match =
        key.match(/^image(?:_url)?_(\d+)$/i) ||
        key.match(/^image_base64_(\d+)$/i);
...
function normalizeDoubaoArgs(rawArgs) {
  ...
  const wantsCompose =
    rawCommand.includes("compose") || rawCommand.includes("合成");
  const wantsEdit =
    rawCommand.includes("edit") ||
    rawCommand.includes("image2image") ||
    rawCommand.includes("i2i") ||
    rawCommand.includes("修图") ||
    rawCommand.includes("改图");
  const wantsGenerate =
    rawCommand.includes("generate") ||
    rawCommand.includes("txt2img") ||
    rawCommand.includes("t2i") ||
    rawCommand.includes("生成");

  if (
    wantsCompose ||
    (wantsEdit && collectedImages.length > 1) ||
    (!wantsGenerate && collectedImages.length > 1)
  ) {
    args.command = "DoubaoComposeImage";
  } else if (
    wantsEdit ||
    (wantsCompose && collectedImages.length === 1) ||
    (!wantsGenerate && collectedImages.length === 1)
  ) {
...
function isValidDoubaoGenArgs(args) {
  ...
  const isResolutionValid = (res, command) => {
    ...
    if (
      upperRes === "ADAPTIVE" &&
      (command === "DoubaoEditImage" || command === "DoubaoComposeImage")
    )
      return true;
  ...
  if (args.command === "DoubaoGenerateImage") {
    ...
    if (
      !(
        ...
        (typeof args.image_base64 === "string" && args.image_base64.trim())
      )
    )
      return false;
    if (!isResolutionValid(args.resolution, args.command)) return false;
...
async function signRequest() {
  if (!VOLCENGINE_API_KEY) {
    ...
    console.warn(
      "[DoubaoGen Plugin] WARN: API_KEY (e.g., VOLCENGINE_API_KEY in config.env) is not set. Authentication will likely fail."
    );
...
async function getImageData(imageUrl, imageBase64) {
    ...
    try {
      ...
      if (e.code === "ENOENT") {
        ...
        const structuredError = new Error(
          `File not found locally, requesting remote fetch for: ${imageUrl}`
        );
  ...
  throw new Error(
    `Unsupported image format or protocol. Please use an https:// or file:// URL.`
  );
}
...
async function generateImageAndSave(args) {
  ...
  if (!VOLCENGINE_API_KEY) {
    throw new Error(
      "DoubaoGen Plugin Error: API_KEY (e.g., VOLCENGINE_API_KEY in config.env) environment variable is required."
    );
  }
  if (!PROJECT_BASE_PATH) {
    throw new Error(
      "DoubaoGen Plugin Error: PROJECT_BASE_PATH environment variable is required for saving images."
    );
  }
  if (!SERVER_PORT) {
    throw new Error(
      "DoubaoGen Plugin Error: SERVER_PORT environment variable is required for constructing image URL."
    );
  }
  if (!IMAGESERVER_IMAGE_KEY) {
    throw new Error(
      "DoubaoGen Plugin Error: IMAGESERVER_IMAGE_KEY environment variable is required for constructing image URL."
    );
  }
  if (!VAR_HTTP_URL) {
    throw new Error(
      "DoubaoGen Plugin Error: VarHttpUrl environment variable is required for constructing image URL."
    );
  }
  ...
  if (!command) {
    throw new Error(
      `DoubaoGen Plugin Error: 'command' not specified in arguments.`
    );
  }

  if (!isValidDoubaoGenArgs(args)) {
    throw new Error(
      `DoubaoGen Plugin Error: Invalid arguments for command '${command}': ${JSON.stringify(
        args
      )}.`
    );
  }
  ...
  if (command === "DoubaoEditImage") {
    ...
    if (!imageData) {
      ...
      throw new Error(
        "DoubaoGen Plugin Error: 'image' parameter is required and must be processed for the DoubaoEditImage command."
      );
    }
  } else if (command === "DoubaoComposeImage") {
    const imageKeys = Object.keys(args).filter((k) =>
      /^image(?:_base64)?_\d+$/i.test(k)
    );

    const indices = imageKeys
      .map((k) => {
    ...
    if (indices.length === 0) {
      throw new Error(
        "DoubaoGen Plugin Error: For DoubaoComposeImage, at least one 'image_N' or 'image_base64_N' (N>0) parameter is required."
      );
    }
    ...
    for (let i = 1; i <= maxIndex; i++) {
      ...
      if (!imageUrl && !imageBase64) {
        throw new Error(
          `DoubaoGen Plugin Error: Image parameters are not continuous. Missing 'image_${i}' or 'image_base64_${i}'.`
        );
      }
      ...
      try {
        ...
        if (e.code === "FILE_NOT_FOUND_LOCALLY") {
          const enhancedError = new Error(
            `In multi-image composition, image ${i} (parameter: ${imageUrlKey}) was not found locally and requires remote fetching.`
          );
        ...
        throw new Error(
          `Error processing image ${i} ('${imageUrlKey}'): ${e.message}`
        );
      }
    }
  }
  ...
  const response = await dmxApiAxiosInstance.post(fullApiUrl, payload);
  ...
  if (!generatedImageUrlOrBase64) {
    throw new Error(
      "DoubaoGen Plugin Error: Failed to extract image data/URL from DMXAPI response. Response: " +
  ...
  if (generatedImageUrlOrBase64.startsWith("http")) {
    ...
    if (contentType && contentType.startsWith("image/")) {
      ...
      const urlExtMatch =
        generatedImageUrlOrBase64.match(/\.([^.?]+)(?:[?#]|$)/);
  ...
  const relativeServerPathForUrl = path
    .join("doubaogen", generatedFileName)
    .replace(/\\\\/g, "/");
  ...
  const result = {
    content: content,
    details: {
      // Keep details for logging or other purposes if needed
  ...
  try {
    if (!inputData.trim()) {
      console.log(
        JSON.stringify({
          status: "error",
          error: "DoubaoGen Plugin Error: No input data received from stdin.",
        })
      );
      process.exit(1);
      return;
    ...
    if (e.code === "FILE_NOT_FOUND_LOCALLY") {
      ...
      const finalErrorMessage = detailedError.startsWith(
        "DoubaoGen Plugin Error:"
      )
        ? detailedError
        : `DoubaoGen Plugin Error: ${detailedError}`;
      console.log(
        JSON.stringify({ status: "error", error: finalErrorMessage })
      );
    }
    process.exit(1);
  }
