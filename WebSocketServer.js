    ...
    upgradeHandler = (request, socket, head) => {
        ...
        if (clientType === 'VChatSync') {
            if (typeof vchatSyncAuthValidator !== 'function') {
                writeLog('VChatSync connection denied. Auth validator is not registered.');
                socket.destroy();
                return;
            }
            try {
    ...
    wssInstance.on('connection', (ws, request) => {
        ...
        ws.on('close', () => {
            if (ws.clientType === 'DistributedServer') {
              ...
              writeLog(`VChatSync latest_seq client ${ws.clientId} disconnected and removed.`);
           } else {
...
function broadcastVChatSyncLatestSeq(latestSeq, detail = {}) {
    ...
    writeLog(`Broadcasted VChatSync latest_seq=${latestSeq} to ${sent} client(s).`);
    return sent;
}
...
function registerVChatSyncAuthValidator(validator) {
    ...
    if (serverConfig.debugMode) {
        console.log(`[WebSocketServer] VChatSync auth validator ${vchatSyncAuthValidator ? 'registered' : 'cleared'}.`);
    }
}
